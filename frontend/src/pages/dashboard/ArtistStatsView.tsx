/**
 * @file ArtistStatsView.tsx
 * @description Artist's personal stats — commission, KPIs, service breakdown, trend.
 *
 * Rendered as a sub-route inside ArtistDashboard (the layout wrapper).
 * The outer header/sidebar/sign-out are handled by DashboardLayout.
 */
...
interface TimePerformance {
  schedulableVisits: number;
  nonSchedulableVisits: number;
...
// ── Component ────────────────────────────────────────────────────────────────
export default function ArtistStatsView() {
  // Date range
  ...
  const fetchDashboardData = useCallback(async () => {
    try {
      ...
      const [profileRes, summaryRes, servicesRes, trendRes, timePerfRes] =
        await Promise.all([
          fetch(`${API}/api/artist-dashboard/profile`, {
            credentials: "include",
            cache: "no-store",
          }),
          fetch(`${API}/api/artist-dashboard/summary?${qs}`, {
            credentials: "include",
            cache: "no-store",
          }),
          fetch(`${API}/api/artist-dashboard/services?${qs}`, {
            credentials: "include",
            cache: "no-store",
          }),
          fetch(`${API}/api/artist-dashboard/daily-trend?${qs}`, {
            credentials: "include",
            cache: "no-store",
          }),
          fetch(`${API}/api/artist-dashboard/time-performance?${qs}`, {
            credentials: "include",
            cache: "no-store",
          }),
        ]);
            ...
            <div className="relative z-10 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              <div>
                ...
                <div className="flex items-baseline gap-3">
                  ...
                  <span className="text-lg font-bold text-yellow-200/80">
                    earned
                  </span>
                </div>
              ...
              <div className="flex flex-col items-center gap-1 bg-white/15 backdrop-blur-sm rounded-xl px-6 py-4">
                ...
                <span className="text-xs text-yellow-100/80 font-medium uppercase tracking-wide">
                  Rate
                </span>
              </div>
            </div>
      ...
      <div className="mb-6 flex flex-wrap items-center gap-2">
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          <button
            ...
            className="w-9 h-9 flex items-center justify-center rounded-lg border border-stone-200 bg-white text-stone-500 hover:bg-stone-50 transition-colors"
            title="Refresh now"
          >
            ...
            },
            {
              ...
              value:
                summary.hoursWorked > 0
                  ? formatCurrency(
                      Math.round(
                        summary.totalRevenue / summary.hoursWorked
                      )
                    )
                  : "—",
            ...
            <div className="space-y-2 max-h-80 overflow-y-auto pr-1">
              {trend.map((d) => {
                const revPct = Math.max(
                  (d.revenue / maxTrendRevenue) * 100,
                  2
                );
                const commPct = Math.max(
                  (d.commission / maxTrendRevenue) * 100,
                  1
                );
              ...
              <div className="flex items-center gap-4 pt-2 border-t border-stone-100 mt-2">
                ...
                <span className="flex items-center gap-1.5 text-xs text-amber-600">
                  <span className="w-3 h-2 rounded bg-amber-400" /> Your
                  Commission
                </span>
              </div>
            </div>
          ...
          return (
            ...
            >
                ...
                <div className="flex-1 grid grid-cols-2 gap-3">
                  <div className="rounded-xl bg-stone-50 border border-stone-100 px-4 py-3 text-center">
                    <p className="text-lg font-bold text-stone-900">
                      {timePerf.schedulableVisits}
                    </p>
                    <p className="text-xs text-stone-500">
                      Visits with duration data
                    </p>
                  </div>
                  {timePerf.nonSchedulableVisits > 0 && (
                    <div className="rounded-xl bg-stone-50 border border-stone-100 px-4 py-3 text-center">
                      <p className="text-lg font-bold text-stone-400">
                        {timePerf.nonSchedulableVisits}
                      </p>
                      <p className="text-xs text-stone-400">
                        Without duration (skipped)
                      </p>
                    </div>
                  )}
                </div>
              ...
              <p className="text-xs text-stone-400 mb-4">
                Tolerance ±10 min — only time beyond that counts as
                over/under.
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-stone-200">
                        <th className="text-left py-2 px-2 text-stone-500 font-medium text-xs">
                          Date
                        </th>
                        <th className="text-right py-2 px-2 text-stone-500 font-medium text-xs">
                          Expected
                        </th>
                        <th className="text-right py-2 px-2 text-stone-500 font-medium text-xs">
                          Actual
                      ...
                      {timePerf.perVisit
                        ...
                        .map((v, i) => (
                          ...
                          >
                            ...
                            <td className="py-2 px-2 text-right text-stone-600 text-xs">
                              {v.expectedMins}m
                            </td>
                            <td className="py-2 px-2 text-right text-stone-600 text-xs">
                              {v.actualMins}m
